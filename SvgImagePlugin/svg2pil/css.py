...
#import cssselect2
from .cssselect2 import Matcher, compile_selector_list
#import tinycss2
from .tinycss2 import parse_stylesheet, parse_one_component_value, parse_declaration_list, serialize
    ...
    for rule in stylesheet_rules:
        if rule.type == 'at-rule':
            if rule.lower_at_keyword == 'import' and rule.content is None:
                ...
                url_token = parse_one_component_value(rule.prelude)
...
def parse_declarations(input):
    ...
    for declaration in parse_declaration_list(input):
        # TODO: warn on error
...
def parse_stylesheets(tree, url):
    ...
    """
    normal_matcher = Matcher()
    important_matcher = Matcher()
    for stylesheet in find_stylesheets(tree):
        for rule in find_stylesheets_rules(tree, stylesheet, url):
            normal_declarations, important_declarations = parse_declarations(
                rule.content)
            for selector in compile_selector_list(rule.prelude):
